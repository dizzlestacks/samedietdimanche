...
function activateTab(name) {
  ...
  if (name === 'waitlist' && !waitlistLoaded) loadWaitlist();
}
...
function renderProducts() {
  ...
  const items = products.filter(p => !q ||
    (p.name || '').toLowerCase().includes(q) ||
    (p.description || '').toLowerCase().includes(q));
  productList.innerHTML = '';
  if (items.length === 0) {
    productList.innerHTML = '<p class="loading-text">' +
...
function renderBudItems() {
  const q = budSearch.trim().toLowerCase();
  const items = budItems.filter(i => !q ||
    (i.name || '').toLowerCase().includes(q) ||
    (i.category || '').toLowerCase().includes(q) ||
    (i.strain || '').toLowerCase().includes(q));
  budList.innerHTML = '';
  if (items.length === 0) {
    budList.innerHTML = '<p class="loading-text">' +
      (q ? 'No items match your search.' : 'No menu items yet. Click "+ Add Item" to create one.') +
      '</p>';
    return;
  }
  items.forEach(item => {
...
if (productSearchInput) {
  productSearchInput.addEventListener('input', (e) => { productSearch = e.target.value; renderProducts(); });
}
const budSearchInput = document.getElementById('bud-search');
if (budSearchInput) {
  budSearchInput.addEventListener('input', (e) => { budSearch = e.target.value; renderBudItems(); });
}
...
function renderDashboard() {
  ...
  setText('stat-wl-total', waitlistEntries.length);
  setText('stat-wl-watches', waitlistEntries.filter(e => e.category === 'watches').length);
  setText('stat-wl-bud', waitlistEntries.filter(e => e.category === 'bud').length);
  setText('stat-products', products.filter(p => p.active).length);
  setText('stat-bud-items', budItems.filter(i => i.active).length);
  ...
  const rows = [...waitlistEntries]
    ...
    .slice(0, 6);
  if (rows.length === 0) {
    recent.innerHTML = '<p class="loading-text">No signups yet.</p>';
    return;
  }
  recent.innerHTML = rows.map(e => `
    <div class="recent-row">
      <span class="recent-email">${escapeHtml(e.email)}</span>
      <span class="recent-meta">
        <span class="wl-cat-badge wl-cat-${escapeHtml(e.category)}">${escapeHtml(e.category)}</span>
        <span class="recent-date">${escapeHtml(formatDate(e.created_at))}</span>
      </span>
    </div>
  `).join('');
}
...
async function loadWaitlist() {
  ...
  try {
    ...
    waitlistEntries = Array.isArray(payload) ? payload : (payload.data || []);
    waitlistLoaded = true;
    renderWaitlist();
    renderDashboard();
  } catch (err) {
